...
def _complete_fallback_io_fields(skill: SkillNode, *, role: str) -> list[InterfaceField]:
    fields = _fallback_io_fields(skill, role=role)
    if fields:
        return fields
    return _semantic_fallback_io_fields(skill, role=role)
...
def _semantic_fallback_io_fields(skill: SkillNode, *, role: str) -> list[InterfaceField]:
    text = _skill_text_for_classification(skill)
    if role == "input":
        return _semantic_fallback_inputs(skill, text)
    if role == "output":
        return _semantic_fallback_outputs(skill, text)
    return []
...
def _semantic_fallback_inputs(skill: SkillNode, text: str) -> list[InterfaceField]:
    fields: list[InterfaceField] = []
    if _has_semantic_term(
        text,
        (
            "analytics",
            "analysis",
        ...
        fields.append(
            _semantic_field(
                skill,
                name="source_data_context",
                kind="data",
                description="Source data, metrics, or analytical context consumed by the skill.",
                confidence=0.48,
                evidence_terms=(
                    "data",
                    "analytics",
    ...
    if _has_semantic_term(
        ...
        fields.append(
            _semantic_field(
                ...
                kind="data",
                description="Role, compensation, jurisdiction, party, and policy details needed for employment documents.",
                confidence=0.5,
                evidence_terms=(
                    "employment",
                    "contract",
    ...
    if _has_semantic_term(
        ...
        fields.append(
            _semantic_field(
                ...
                kind="planning_state",
                description="Audience, purpose, required sections, and constraints for the generated content.",
                confidence=0.45,
                evidence_terms=(
                    "document",
                    "documents",
...
def _semantic_fallback_outputs(skill: SkillNode, text: str) -> list[InterfaceField]:
    fields: list[InterfaceField] = []
    if _has_semantic_term(
        text,
        (
        ...
        fields.append(
            _semantic_field(
                ...
                kind="artifact",
                description="Draft employment contract, offer letter, handbook, or HR policy document.",
                confidence=0.52,
                evidence_terms=(
                    "employment",
                    "contract",
    ...
    ) and _has_semantic_term(
        text,
        ("analytics", "data", "metric", "metrics", "trend", "trends"),
    ):
        fields.append(
            _semantic_field(
                skill,
                name="data_narrative",
                kind="report",
                description="Narrative structure, recommendations, and stakeholder-facing interpretation of data.",
                confidence=0.5,
                evidence_terms=(
                    "narrative",
                    "narratives",
    ...
    if _has_semantic_term(
        ...
        fields.append(
            _semantic_field(
                ...
                name="draft_document",
                kind="report",
                description="Drafted document, report, policy, guideline, or template produced by the skill.",
                confidence=0.46,
                evidence_terms=(
                    "document",
                    "documents",
...
def _semantic_field(
    ...
    *,
    ...
    return InterfaceField(
        ...
        inferred=True,
        evidence=_semantic_evidence(skill, evidence_terms),
    )


def _dedupe_fields(fields: list[InterfaceField]) -> list[InterfaceField]:
    output: list[InterfaceField] = []
...
def _semantic_evidence(skill: SkillNode, terms: tuple[str, ...]) -> list[InterfaceEvidence]:
    seen: set[tuple[int, str]] = set()
    evidence: list[InterfaceEvidence] = []
    for line_number, line in enumerate(skill.raw_text.splitlines(), start=1):
        if _has_semantic_term(line.lower(), terms):
            _append_interface_evidence(evidence, seen, line_number, line, skill.id)
        if len(evidence) >= 3:
            break
    if not evidence and skill.description:
        evidence.append(InterfaceEvidence(skill=skill.id, line=0, text=skill.description))
    return evidence[:3]
