...
def agent_run_spec_from_workflow_plan(route: RouteResult, workflow_plan: dict[str, Any]) -> AgentRunSpec:
    """Build an AgentRunSpec from a planner-authored workflow plan.

    The router decides which skills are allowed. The planner may organize those
    skills into task-specific phases, but it must not introduce new skills.
    """

    draft = agent_run_spec_from_route(route)
    selected_ids = {skill.skill_id for skill in route.selected_skills}
    phases = _phases_from_workflow_plan(workflow_plan, selected_ids)
    constraints = _dedupe([*draft.constraints, *[str(item) for item in workflow_plan.get("constraints", []) if str(item).strip()]])
    strategy = _execution_strategy_from_route(route, phases, draft.acceptance_criteria)
    strategy_payload = workflow_plan.get("execution_strategy", {})
    if isinstance(strategy_payload, dict):
        parallelization = str(strategy_payload.get("parallelization_policy") or strategy_payload.get("parallelization") or "").strip()
        delegation = str(strategy_payload.get("delegation_policy") or strategy_payload.get("delegation") or "").strip()
        verification = str(strategy_payload.get("verification_policy") or strategy_payload.get("verification") or "").strip()
        if parallelization:
            strategy.parallelization_policy = parallelization
        if delegation:
            strategy.delegation_policy = delegation
        if verification:
            strategy.verification_policy = verification
    return AgentRunSpec(
        objective=str(workflow_plan.get("objective") or route.query),
        selected_skills=draft.selected_skills,
        phases=phases,
        execution_strategy=strategy,
        required_order=draft.required_order,
        constraints=constraints,
        acceptance_criteria=draft.acceptance_criteria,
    )


def _phases_from_workflow_plan(workflow_plan: dict[str, Any], selected_ids: set[str]) -> list[AgentRunPhase]:
    raw_phases = workflow_plan.get("phases", [])
    if not isinstance(raw_phases, list) or not raw_phases:
        raise ValueError("workflow_plan.phases must be a non-empty list")
    phases: list[AgentRunPhase] = []
    for index, raw in enumerate(raw_phases, start=1):
        if not isinstance(raw, dict):
            raise ValueError("workflow_plan.phases items must be objects")
        phase_id = str(raw.get("id") or f"phase_{index}").strip()
        if not phase_id:
            raise ValueError("workflow_plan phase id cannot be empty")
        skill_ids = [str(item) for item in raw.get("skill_ids", []) if str(item).strip()]
        unknown = sorted(set(skill_ids) - selected_ids)
        if unknown:
            raise ValueError(f"workflow_plan phase uses skills not selected by route: {', '.join(unknown)}")
        if not skill_ids:
            raise ValueError(f"workflow_plan phase {phase_id} must reference at least one selected skill")
        phases.append(
            AgentRunPhase(
                id=phase_id,
                goal=str(raw.get("goal") or "Apply selected skill guidance to the task."),
                skill_ids=skill_ids,
                ...
                expected_outputs=[str(item) for item in raw.get("expected_outputs", []) if str(item).strip()],
    ...
    for phase in phases:
        unknown_deps = sorted(set(phase.depends_on) - phase_ids)
        if unknown_deps:
            raise ValueError(f"workflow_plan phase {phase.id} depends on unknown phases: {', '.join(unknown_deps)}")
        if phase.id in phase.depends_on:
            raise ValueError(f"workflow_plan phase {phase.id} cannot depend on itself")
    return phases
