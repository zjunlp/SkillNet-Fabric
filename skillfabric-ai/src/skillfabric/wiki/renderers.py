...
from skillfabric.wiki.models import WikiBuildConfig, WikiPage
...
def _skill_page(
    ...
    capability = _one_line(contract.capability if contract else skill.description)
    when_to_use = _one_line(contract.when_to_use if contract else skill.description)
...
def _directed_link_label(edge: Edge, current_skill_id: str) -> tuple[str, str]:
    ...
    if edge.type == "compose_with":
        if edge.source == current_skill_id:
            return "precedes", edge.target
        return "follows", edge.source
