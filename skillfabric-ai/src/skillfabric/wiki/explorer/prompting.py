...
def render_system_prompt(context: ExplorerPromptContext) -> str:
    ...
    return (
        ...
        "- query_wiki contents: manifest.json, index.md, EXPLORER.md, page_index.jsonl, skills/, skills/source/, edges/, workflows/, communities/.\n\n"
        "# Output\n\n"
        ...
        "Step 3: Read skills/index.md or manifest.json only if index.md does not expose enough candidate metadata.\n"
        "Step 4: Read skills/*.md card pages for plausible candidates before selecting or rejecting them.\n"
        "Step 5: Read skills/source/*.md only when a candidate card is insufficient to resolve routing boundaries, critical policy, tools, prerequisites, or execution constraints.\n"
        ...
        "Step 8: Read community pages only as tie-breakers or clustering context; they are not enough evidence by themselves.\n"
        "Step 9: Select a small, useful, low-redundancy package and stop when additional reads are unlikely to change the selection.\n\n"
        "# Tool Protocol\n\n"
        ...
        "- Do not read skills/source/*.md by default. Full source is authoritative but expensive; use it only when the card cannot answer a routing-critical question.\n"
        ...
        "- Use relative paths such as skills/example.md, skills/source/example.md, edges/bridge_edges.jsonl, or workflows/example.md.\n"
...
def render_user_prompt(context: ExplorerPromptContext) -> str:
    ...
    return (
        ...
        "Read index.md first. Prefer skills/*.md card pages before skills/source/*.md "
