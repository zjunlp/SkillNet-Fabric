...
def _relation_semantics() -> str:
    return "\n".join(
        [
            ...
            "For depend_on, both endpoint sources must establish the producer and consumer roles. Semantic compatibility, not literal field-name equality, is required, and the endpoints need not name the other skill.",
            "Compatibility is specific enough when the target explicitly names the concrete format, schema, protocol, state, or interface, or when both sources describe the same role-specific payload in enough detail to show a direct handoff.",
            "When the source explicitly emits a named format, schema, protocol, or state and the target explicitly reads that same interface, specificity is satisfied even if the target supports additional formats or upstream producers.",
            "When both sides prove that exact named-interface match and the target actively consumes it on the claimed path, classify the handoff as depend_on unless the source says the result is unavailable.",
            "Broad labels such as artifact, file, data, image, code, or content do not prove compatibility by themselves.",
            "The target's selected operation must actually read, transform, inspect, validate, or continue from the source result. Merely uploading, displaying, quoting, or being guided by it is insufficient.",
            "Advice, reference material, style rules, or best practices may guide execution but do not consume an output. Test such a pair as compose_with only when a direct adjacent workflow is supported; otherwise use none.",
            "A Skill that supplies a theme, policy, pattern, or configuration normally produces guidance for an artifact-producing Skill. Do not reverse the handoff direction merely because the guidance says it can be applied to an artifact. Treat it as a downstream consumer only when source evidence shows an actual read-and-rewrite or post-processing mechanism for an existing artifact.",
            "Words such as apply, style, or format describe intent but do not prove that mechanism. Require evidence that the Skill opens or parses the existing artifact and writes a modified artifact.",
            "Helpful context is not a hard input. On the claimed path, the target operation must be unable to execute correctly without receiving the concrete source result.",
            ...
            "The same carrier type, such as text, image, report, code, or structured data, does not make different task objectives similar_to.",
...
def _decision_process() -> str:
    return "\n".join(
        [
            ...
            "3. Before rejecting depend_on, compare skill_a.produces against skill_b.requires and skill_b.produces against skill_a.requires, then verify both possible handoffs against source evidence. Canonical pair order has no semantic direction.",
            "4. Accept depend_on only when source evidence establishes a concrete output or state, target evidence establishes a specific compatible input and active consumer operation, and the target needs that result on the claimed path. Do not require identical wording. Store source_skill -> target_skill in execution order.",
...
def _relation_examples() -> str:
    return "\n".join(
        [
            ...
            "- depend_on: document-author explicitly produces a DOCX file and a generic document-converter explicitly accepts DOCX input; different field names do not weaken the handoff.",
            "- depend_on: session-bootstrap establishes an authenticated browser session and form-submitter explicitly requires an existing authenticated session.",
            "- depend_on: deck-post-processor opens an existing deck and writes a restyled deck; its reader-and-writer workflow proves that it consumes the input artifact.",
            ...
            "- compose_with: feature-implementer produces code changes and a general code-review stage inspects them; review is adjacent, but the implementation is not a target-specific hard prerequisite.",
            ...
            "- none: an image producer exports PNG while a style advisor merely discusses images; the shared format does not establish a consumer role.",
            "- none: a palette-and-font catalog says its guidance applies to any artifact but has no reader or writer for the artifact's format; applicability alone does not make it a downstream consumer.",
            "- none: a proposal writer produces a file and a browser tool can upload arbitrary files; transport capability alone is not a semantic handoff.",
            "- none: a report-writing workflow includes one optional chart while a visualization specialist independently builds charts; the embedded substep is not a comparable end-to-end alternative.",
